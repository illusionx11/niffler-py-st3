...
class RegisterPage(BasePage):
    ...
    def should_be_register_heading(self):
        header = self.browser.find_element(*RegisterPage.Locators.REGISTER_HEADER)
        assert header.is_displayed()
        assert header.text.lower() == "sign up"
        
    def should_be_register_form(self):
        assert self.is_element_present(*RegisterPage.Locators.REGISTER_FORM)
        assert self.is_element_present(*RegisterPage.Locators.PASSWORD_REPEAT_INPUT)
        
    def should_be_register_button(self):
        assert self.is_element_present(*RegisterPage.Locators.SUBMIT_BTN)
        
    def register_user(self, username: str, password: str, password_repeat: str | None = None):
        username_input = self.browser.find_element(*RegisterPage.Locators.USERNAME_INPUT)
        username_input.send_keys(username)
        password_input = self.browser.find_element(*RegisterPage.Locators.PASSWORD_INPUT)
        password_input.send_keys(password)
        password_repeat_input = self.browser.find_element(*RegisterPage.Locators.PASSWORD_REPEAT_INPUT)
        if password_repeat is None:
            password_repeat = password
        password_repeat_input.send_keys(password_repeat)
        self.browser.find_element(*RegisterPage.Locators.SUBMIT_BTN).click()
    ...
    def should_be_errors_in_validation(self, errors: dict[str, list[ValidationErrors]]):
        if "username" in errors and len(errors["username"]) > 0:
            username_input = WebDriverWait(self.browser, 10).until(EC.presence_of_element_located(RegisterPage.Locators.USERNAME_INPUT))
        ...
        if "password" in errors and len(errors["password"]) > 0:
            password_input = WebDriverWait(self.browser, 10).until(EC.presence_of_element_located(RegisterPage.Locators.PASSWORD_INPUT))
        ...
        if "password_repeat" in errors and len(errors["password_repeat"]) > 0:
            password_repeat_input = WebDriverWait(self.browser, 10).until(EC.presence_of_element_located(RegisterPage.Locators.PASSWORD_REPEAT_INPUT))
    ...
    def should_be_successful_registration(self):
        assert self.is_element_present(*RegisterPage.Locators.REGISTER_SUCCESS_HEADER)
        assert self.is_element_present(*RegisterPage.Locators.REGISTER_SUCCESS_LOGIN_BTN)