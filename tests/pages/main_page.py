...
class MainPage(BasePage):
    ...
    def should_be_spendings_div(self):
        assert self.is_element_present(*MainPage.Locators.SPENDINGS_DIV)
        
    def should_be_stats_div(self):
        assert self.is_element_present(*MainPage.Locators.STATS_DIV)
        
    def add_new_spending(self, data: dict):
        self.browser.find_element(*MainPage.Locators.ADD_SPENDING_BTN).click()
        amount_input = self.browser.find_element(*MainPage.Locators.AMOUNT_INPUT)
        amount_input.send_keys(Keys.CONTROL + "a")
        amount_input.send_keys(Keys.BACKSPACE)
        amount_input.send_keys(data["amount"])
        self.browser.find_element(*MainPage.Locators.CURRENCY_SELECT).click()
        all_currencies = self.browser.find_elements(*MainPage.Locators.ALL_CURRENCIES)
        for currency in all_currencies:
            if currency.get_attribute("data-value") == data["currency"]:
                currency.click()
                break
        self.browser.find_element(*MainPage.Locators.CATEGORY_INPUT).send_keys(data["category"])
        self.browser.find_element(*MainPage.Locators.DESCRIPTION_INPUT).send_keys(data["description"])
        self.browser.find_element(*MainPage.Locators.SAVE_BTN).click()
        
    def should_be_new_spending_in_table(self, data: dict):
        
        is_spending_added = False
        table_rows = WebDriverWait(self.browser, 10).until(EC.presence_of_all_elements_located(MainPage.Locators.SPENDINGS_TABLE_ROWS))
    ...
    def remove_spendings(self, indexes: list[int]):
        ...
        for index in indexes:
            ...
            row_to_delete.find_element(*MainPage.Locators.SPENDING_CHECKBOX).click()
        delete_btn = WebDriverWait(self.browser, 10).until(EC.element_to_be_clickable(MainPage.Locators.DELETE_SPENDING_BTN))
        assert delete_btn.is_enabled()
        delete_btn.click()
        dialog_div = self.browser.find_element(*MainPage.Locators.DELETE_MODAL_DIV)
    ...
    def should_not_be_deleted_spendings(self, spendings: list[dict], indexes: list[int]):
        new_table_rows = WebDriverWait(self.browser, 10).until(EC.presence_of_all_elements_located(MainPage.Locators.SPENDINGS_TABLE_ROWS))
    ...
    def should_be_errors_in_validation(self, errors: dict[str, list[ValidationErrors]]):
        if "amount" in errors and len(errors["amount"]) > 0:
            amount_input = WebDriverWait(self.browser, 10).until(EC.presence_of_element_located(MainPage.Locators.AMOUNT_INPUT))
        ...
        if "category" in errors and len(errors["category"]) > 0:
            category_input = WebDriverWait(self.browser, 10).until(EC.presence_of_element_located(MainPage.Locators.CATEGORY_INPUT))
    ...
    def make_search(self, query: str):
        search_input = self.browser.find_element(*MainPage.Locators.SEARCH_INPUT)
    ...
    def should_be_no_search_results(self):
        assert self.is_element_not_present(*MainPage.Locators.SPENDINGS_TABLE), "Ожидалось, что поиск не даст результатов, но они есть"